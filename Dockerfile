...
# rocwmma-version.hpp is generated by cmake configure from a .in template
RUN if [ "${BUILD_TYPE}" = "hipblas" ]; then \
        git clone --depth 1 https://github.com/ROCm/rocWMMA /tmp/rocwmma && \
        cmake -S /tmp/rocwmma -B /tmp/rocwmma-build \
            -DCMAKE_INSTALL_PREFIX=/opt/rocm \
            -DAMDGPU_TARGETS=gfx1151 \
            -DROCWMMA_BUILD_TESTS=OFF \
            -DROCWMMA_BUILD_SAMPLES=OFF \
            -DROCWMMA_BUILD_DOCS=OFF \
            -DROCWMMA_BUILD_VALIDATION_TESTS=OFF \
            -DROCWMMA_BUILD_BENCHMARK_TESTS=OFF && \
        cmake --install /tmp/rocwmma-build --component rocwmma-dev 2>/dev/null || \
        ( cp -r /tmp/rocwmma/library/include/rocwmma /opt/rocm/include/ && \
          find /tmp/rocwmma-build -name 'rocwmma-version.hpp' -exec cp {} /opt/rocm/include/rocwmma/ \; ) && \
        rm -rf /tmp/rocwmma /tmp/rocwmma-build \
    ; fi
