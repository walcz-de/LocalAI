...
func DefaultRegistry() map[string]FieldMetaOverride {
	...
	return map[string]FieldMetaOverride{
		...
		// --- Agent ---
		"agent.max_generation_tokens": {
			Section:     "mcp",
			Label:       "Agent Max Generation Tokens",
			Description: "Upper bound on tokens the agent may generate per inference step. cogito never sets max_tokens itself, so without a cap an agent can generate until it exhausts the context. 0 falls back to the server default.",
			Component:   "number",
			Order:       132,
		},
